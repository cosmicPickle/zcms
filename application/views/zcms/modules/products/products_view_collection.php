...
$pd_list = $product->get_product_list();
...
<ul class="categories<?php if($is_vertical) echo " vertical-gallery-categories" ?>">
    <?php 
        foreach($pd_list as $pdc) :
    ?>
    <li>
        <a <?php if($pdc->code == $pd->code) echo "class='active'"; ?> href="<?php echo $this->zcms->frontend().'product_collection?code='.$pdc->code; ?>">
        <?php echo $pdc->title; ?>
        </a>
    </li>
