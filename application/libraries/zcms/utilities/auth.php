...
class Auth extends ZCMS {
    ...
    public function __construct()
    {
        ...
        if($this->router->fetch_class() == self::AUTH_CONTROLLER)
        {
            ...
            if($this->_check_session() && $_method == "login")
        ...
        elseif($this->router->fetch_class() != self::AUTH_CONTROLLER)
        {
            ...
            $this->session_info->image = $this->session->userdata("zcms_user_image");
        }
    }
    ...
    {
        $sess->image = json_decode($sess->image);
        ...
        $this->session->set_userdata("zcms_user_image", $sess->image[0]);
    ...
    private function _logout()
    {
        ...
        $this->session->unset_userdata("zcms_user_image");
