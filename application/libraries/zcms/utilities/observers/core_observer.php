...
class Core_Observer extends Observer {
    ...
    public function zcms_load_headers_before($observer)
    {
        $zcms = $observer->get_data();
        
        $this->db->from("zcms_languages as t1");
        $this->sql->add_lang_table("zcms_languages".$this->translate->get_table_sufix());
        $zcms->languages = $this->db->get()->result();
    }
}
