...
class Pages extends CI_Controller {
        ...
        public function image_delete($id, $file)
        {
            $this->load->library("zcms/zcms");
            $this->zcms->init();

            $this->zcms->interface->load_interface('form'); 
            $this->interface->form->init('zcms_pages', (object)array("where" => array('t1.id' => $id)));
            
            $this->interface->form->get_field('images')->delete_file($file);
            header("location: ". base_url('index.php/backend/pages/edit/'.$id)); 
        }
}
